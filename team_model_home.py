...
class DataPreperation:
    ...
    def onehot(self):
        ...
        #Undersök, istället för droppa, sätta in värden, scalar efter droppa column? Kolla ordning
        #Neural Network- Loss: 20.69904136657715, MAE: 3.529205799102783, r2: 0.3502049446105957
        X_train = X_train.fillna(X_train.mean())
        X_test = X_test.fillna(X_test.mean()) 
        #Försäkra X_train och X_test shape
        X_test = X_test.reindex(columns=X_train.columns, fill_value=0)
        #Testa medelvärde, medianvärden, 0, min ,median

        X_train_scaled = pd.DataFrame(self.scaler.fit_transform(X_train), columns=X_train.columns)
        X_test_scaled = pd.DataFrame(self.scaler.transform(X_test), columns=X_test.columns) #transform istället för fit_transform pga data leakage
...
def prediction_plot(X_test_home, y_test_home, predict_test_home_shots):
