...
class NeuralNetwork:
    def __init__(self):
        ...
        #Bekräftelse på learning rate, dropout layer, kolla early stopping
    ...
    # undersök undersök om y_predict passar för dimentionerna för säkeställa y_test och y_predict, kolla reshape innan r2
    def evaluate(self, X_test, y_test):
        y_predict = self.model.predict(X_test).flatten() #Flatten för säkra shape innan r2
...
def loss_plot(nn_history):
    plt.figure(figsize=(10,7))
    plt.plot(nn_history.history['loss'], label='Loss training')
    plt.ylabel("loss")
    plt.xlabel("epochs")
    plt.legend()
    plt.title('Loss kurva')
    plt.show()

def prediction_plot(y_test_home, predict_test_home_shots, title="Prediktion vs faktiska värden"):
    xAxis = tf.range(0, len(y_test_home))
    y_test = tf.cast(y_test_home.to_numpy(), tf.float32)
    y_predict = tf.cast(predict_test_home_shots, tf.float32)
    plt.figure(figsize=(10, 7))
    plt.scatter(xAxis, y_test, c='g', label="Actual Test Data")
    plt.scatter(xAxis, y_predict, c='r', label="Predictions")
    plt.legend()
    plt.title(title)
    plt.show()


def nn_train_evaluate(X_train_home, X_test_home, y_train_home, y_test_home):
    ...
    loss_plot(nn_history)
    prediction_plot(y_test_home, predict_test_home_shots)
    return predict_test_home_shots

def rf_train_evaluate(X_train_home, X_test_home, y_train_home, y_test_home):
...
def main():
    ...
    '''Kommentera ut antingen nn_predict eller rf_predict för testa fram Neural Network eller Random forest modeller'''
    #nn_predict = nn_train_evaluate(X_train_home, X_test_home, y_train_home, y_test_home)

    rf_predict = rf_train_evaluate(X_train_home, X_test_home, y_train_home, y_test_home)

#skapa funktioner för plots
